// This legacy combat character is now the player's archdemon.
is_archdemon = true;
cultist_name = "";
...
// Keep the selected demon appearance during the day without changing the archdemon lifecycle.
archdemon_visual_apply = function()
{
	if (demon_type == DEMON_TYPE.IMP)
	{
		sprite_index = s_imp;
		image_xscale = 1.2;
		image_yscale = image_xscale;
	}
	else if (demon_type == DEMON_TYPE.WARLOCK)
	{
		sprite_index = s_warlock;
		image_xscale = 1.2;
		image_yscale = image_xscale;
	}
	else if (demon_type == DEMON_TYPE.BRUTE)
	{
		sprite_index = s_brute;
		image_xscale = 1.15;
		image_yscale = image_xscale;
	}
};
